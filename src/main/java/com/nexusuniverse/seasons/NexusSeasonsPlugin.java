...
public class NexusSeasonsPlugin extends JavaPlugin implements NexusSeasonsAPI {
    ...
    public void onEnable() {
        ...
        // Soft-dependency to NexusMorality's Problem+Solution chat messenger (see plugin.yml's
        // softdepend entry, which makes sure NexusMorality is already loaded/enabled by the time
        // resolve() runs below, if it's installed at all). Only meteor-shower impact explosions
        // (and only when meteor-shower.damage-enabled is on) deal real damage to a player anywhere
        // in this plugin's large weather layer, so it's the only thing wired to this.
        this.moralityMessenger = new MoralityMessengerBridge(this);
        moralityMessenger.resolve();

        this.meteorShower = new MeteorShowerManager(this, config, moralityMessenger);
        meteorShower.start();
